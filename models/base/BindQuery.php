...
class BindQuery
{
    ...
    {
        if ((\Yii::$app->id === 'app-coliseum') || ((isset(\Yii::$app->user))&&(\Yii::$app->user->can(Role::ROLE_TEST)))) {
            if (\Yii::$app->user->can('coliseum_permission_see_deleted')) {
                return $this;
            }
            return $this->status(PmStatus::ID_REMOVE, false);
        }
        return $this->status(PmStatus::ID_PUBLISHED);
    }

    /**
     * Filter query by status condition
     *
     * @param int $status type of status. Use constants from PmStatus
    ...
    public function status(int $status, bool $equal = true): self
    {
        $sign = $equal ? '=' : '<>';
        $uniq = uniqid();
        $modelTableName = $this->modelClass::tableName();
        $subquery = "(SELECT $modelTableName.* FROM $modelTableName INNER JOIN \"pm_uids\" ua__$uniq ON ua__$uniq.uid = $modelTableName.uid AND ua__$uniq.id_status $sign $status)";
        return $this->from([$modelTableName => $subquery]);
    }
